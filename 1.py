                ...
                for i in a_index:
                    ...
                    for soupf in soup_find[i].find_all('tr'):
                        ...
                        for db_food in Food.objects.filter(university=user_data.university):
                            if set(db_food.name.strip().split(' ')).issubset(food_name.split(' ')):
                                food_name = db_food.name
                                flag = True
                            elif db_food.name in food_name.strip():
                                food_name = db_food.name
                                flag = True

                        if not flag and food_name != '':
                            newfood = Food()
                            food_name = food_name.split('|')[1].strip()
                            ...
                            for day in chosen_days_lunch:
                                if (daye[0] == day) and (lunch_data[daye] != []):
                                    ...
                                    for food in lunch_data[daye]:
                                        prefered_food = UserPreferableFood.objects.filter(user=user_data.user,
                                                                                          food__name=food[1])
                                        food_list.append((food[0],
                                                          prefered_food[0],
                                                          food[2]))
